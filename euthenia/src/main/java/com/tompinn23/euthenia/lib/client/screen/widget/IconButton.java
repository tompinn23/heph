package com.tompinn23.euthenia.lib.client.screen.widget;
...
public class IconButton extends Button {
    ...
    @Nullable
    private Holder.Reference<SoundEvent> sound;
    ...
    public IconButton(int x, int y, ItemStack stack, Texture texture, Texture hovering, Component text, OnPress onPress, Screen screen) {
        super(x, y, texture.getWidth(), texture.getHeight(), text, onPress, Button.DEFAULT_NARRATION);
    ...
    public void render(PoseStack matrix, int mouseX, int mouseY, float pt) {
        if (this.visible) {
            ...
            if (this.isHovered && !this.hovering.isEmpty()) {
                this.hovering.draw(matrix, this.getX(), this.getY());
            } else {
                this.texture.draw(matrix, this.getX(), this.getY());
            }
            Font f = this.mc.font;
            ...
            if (!s.isEmpty()) {
                ...
                f.draw(matrix, s, this.xOffset + this.getX() + 0.5F + this.width / 2.0F - width / 2.0F, this.yOffset + this.getY() + this.height / 2.0F - 4, color);
            }
            if (!this.stack.isEmpty()) {
                var globalStack = RenderSystem.getModelViewStack();
                globalStack.pushPose();
                Minecraft mc = Minecraft.getInstance();
                globalStack.translate(this.xOffset + this.getX() - 8.0D + this.width / 2.0F, this.yOffset + this.getY() - 8.0D + this.height / 2.0F, 0.0F);
    ...
    public IconButton setSound(@Nullable Holder.Reference<SoundEvent> sound) {
        this.sound = sound;
        return this;
    }
